...
def get_jira_ticket_from_key(key: str, session) -> Optional[JiraTicket]:
    ...
    """
    result = session.query(Ticket, TicketUpdate).filter(
        Ticket.key == key
    ).order_by(TicketUpdate.updated.desc()).first()
...
def persist_jira_ticket(jira_ticket: JiraTicket, session) -> None:
    ...
    ticket_update = TicketUpdate(ticket_id=ticket.id, status=jira_ticket.status.value, updated=jira_ticket.updated)

    if jira_ticket.id is None:
        session.add_all([ticket, ticket_update])
    else:
        ticket_update = session.query(TicketUpdate).filter(
            TicketUpdate.ticket_id == jira_ticket.id
        ).order_by(TicketUpdate.updated.desc()).first()
        import pdb
        pdb.set_trace()
